...
/** @param {number} deadline @param {number} [now] @returns {number} */
export function remainingAt(deadline, now = Date.now()) { return Math.max(0, Math.ceil((deadline - now) / 1000)); }
/** @param {number} seconds @returns {boolean} */
export function isLastMinute(seconds) { return seconds > 0 && seconds <= 60; }
